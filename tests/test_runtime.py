...
class TestCognitiveLayer(unittest.TestCase):
    def test_memory_and_failure_feedback(self):
        ...
        rec = cog.upsert_thesis("AAPL", "Earnings revision momentum", 0.8, 0.9, source_count=4, horizon_days=15)
    ...
    def test_stale_eviction(self):
        ...
        rec = cog.upsert_thesis("MSFT", "Cloud margin expansion", 0.7, 1.0, source_count=3, horizon_days=20)
...
class TestReactiveLayer(unittest.TestCase):
    def test_reactive_analysis_uses_cognitive_calibration(self):
        ...
        cog.upsert_thesis("NVDA", "AI capex cycle", 0.9, 0.8, source_count=5, horizon_days=30)
        cog.record_failure("NVDA", "late-cycle valuation compression", "trend", realized_return_bps=-220)
        layer = ReactiveLayer(cog)

        decision = layer.analyze(AnalysisRequest("NVDA", "analyze ticker", 0.03, 0.85))
        self.assertEqual(decision.tier, "ticker")
        self.assertLess(decision.confidence, 0.85)
        self.assertIn("base", decision.why.confidence_calibration)
